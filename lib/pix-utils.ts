...
function crc16CCITT(str: string): string {
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    crc = ((crc << 8) & 0xFFFF) ^ crcTable[((crc >> 8) ^ c) & 0xFF];
  }
...
export function generateQRCodeUrl(text: string): string {
  ...
  return `https://chart.googleapis.com/chart?cht=qr&chl=${encodedText}&chs=250x250&choe=UTF-8`;
}

/**
 * Gera um QR Code PIX simples usando apenas a chave PIX
 * Esta é uma alternativa mais simples que não requer o formato completo do PIX
...
export function generateSimplePixQRCode(pixKey: string, amount?: number): string {
  ...
  // Para chaves PIX simples, podemos usar diretamente a chave
  let pixText = pixKey;
  
  // Se houver um valor, podemos adicioná-lo como informação adicional
  if (amount && amount > 0) {
    pixText += ` - Valor: R$ ${amount.toFixed(2)}`;
  }
  
  try {
    return generateQRCodeUrl(pixText);
  } catch (error) {
